...
const ReqValidate = (req, res, next) =>{
    ...
    if(name || description || artist || genre || files){
        next()
    }else{
        req.erros.push('Es necesario al menos un item')
    }
}
...
const fileValidate = (req, res, next ) => {
    ...
    if(files && (!files.image && !files.sound)) {
        req.errors.push('debe proporcionar la imagen y la cancion en los formatos aceptados');
        next()
    }else{
        next()
    }
